...
CHttpClientDlg::CHttpClientDlg(CWnd* pParent /*=NULL*/)
    ...
    m_use_proxy(FALSE)
{
    const DWORD STR_SIZE = 128;
    // load URL, proxy server, and authentication settings from HttpClient.INI...

    TCHAR str[STR_SIZE];
	CString str_true;

	str_true.LoadString(IDS_TRUE);

    GetPrivateProfileString( _T("settings"), _T("use_proxy"), _T("false"), str, STR_SIZE, HTTP_CLIENT_INI );
    str[STR_SIZE-1] = 0;

	if (str_true.CompareNoCase(str) == 0)
        m_use_proxy = TRUE;
    else
        m_use_proxy = FALSE;

    GetPrivateProfileString( _T("settings"), _T("url"), _T("http://"), str, STR_SIZE, HTTP_CLIENT_INI );
    str[STR_SIZE-1] = 0;
    m_urlStr = str;

    GetPrivateProfileString( _T("settings"), _T("proxy"), _T(""), str, STR_SIZE, HTTP_CLIENT_INI );
    str[STR_SIZE-1] = 0;
    m_proxy = str;

    GetPrivateProfileString( _T("settings"), _T("proxyport"), _T("80"), str, STR_SIZE, HTTP_CLIENT_INI );
    str[STR_SIZE-1] = 0;
    m_proxy_port = _ttoi(str);

    GetPrivateProfileString( _T("settings"), _T("basic_auth"), _T("false"), str, STR_SIZE, HTTP_CLIENT_INI );
    str[STR_SIZE-1] = 0;
    ...
    GetPrivateProfileString( _T("settings"), _T("ntlm_auth"), _T("false"), str, STR_SIZE, HTTP_CLIENT_INI );
    str[STR_SIZE-1] = 0;
    if (str_true.CompareNoCase(str) == 0)
        m_auth_ntlm = TRUE;
    else
...
bool WINAPI CHttpClientDlg::Callback(DWORD bytes, DWORD_PTR objPtr)
{
